                        ...
                        <tbody>
                            @foreach ($news as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('admin.news.edit', $item->slug)}}" class="btn btn-primary">Ubah</a>
                                        <a class="btn btn-danger text-light" onclick="event.preventDefault();document.getElementById('delete-form-{{ $item->slug }}').submit();">Hapus</a>
                                        <form id="delete-form-{{ $item->slug }}" action="{{route('admin.news.destroy', $item->slug)}}" method="post" class="mx-0">
                                            @method('delete')
                                            @csrf
                                        </form>
                                    </td>
