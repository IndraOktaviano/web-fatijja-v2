            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form
                        ...
                        @if (isset($edit))
                            action="{{ route('admin.banner.update', $edit->id) }}"
                        @else
                        ...
                        <tbody>
                            @foreach ($banners as $key => $item)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>
                                        <img src="{{ asset('assets/images/hero/banner/'.$item->image) }}" width="100" alt="">
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.banner.edit', $item->id) }}" class="btn btn-primary">Ubah</a>
                                        ...
                                        <form id="delete-form-{{ $item->id }}" action="{{route('admin.banner.destroy', $item->id)}}" method="post" class="mx-0">
                                            @method('delete')
                                            @csrf
                                        </form>
                                    </td>
